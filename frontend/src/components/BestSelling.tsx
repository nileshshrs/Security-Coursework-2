...
export default function BestSelling() {
  ...
  const womenBestsellers = useMemo(() => {
    return getRandomSubset(
      clothes.filter(
        (item) =>
      ...
      <Tabs defaultValue="men" className="w-full">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-6">
            <h2 className="text-2xl font-bold tracking-tight">Shop Bestsellers</h2>
            <TabsList>
        ...
        <TabsContent value="men">
          ...
          ) : menBestsellers.length === 0 ? (
            <div className="w-full flex justify-center py-12">
              <span className="text-gray-400 text-lg">No men's bestsellers.</span>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-6">
              {menBestsellers.map((item) => (
                <Link
                  to={`/clothes/${item._id}`}
                  key={item._id}
        ...
        <TabsContent value="women">
          ...
          ) : womenBestsellers.length === 0 ? (
            <div className="w-full flex justify-center py-12">
              <span className="text-gray-400 text-lg">No women's bestsellers.</span>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-6">
              {womenBestsellers.map((item) => (
                <Link
                  to={`/clothes/${item._id}`}
                  key={item._id}
                  className="group block"
